...
class ArticleController extends BaseController {
    ...
    async getAll(req: Request, res: Response, next: NextFunction){
        const articles = await this.services.articleService.getAll();
    ...
    public async uploadFiles(req: Request, res: Response): Promise<IUploadedPhoto[]> {
        if (!req.files || Object.keys(req.files).length === 0) {
            res.status(400).send("You must upload at lease one and a maximum of " + Config.fileUpload.maxFiles + " photos." + req.files);
            return [];
        }
    ...
    public async add(req: Request, res: Response) {
        ...
        try{
            ...
            return{
                isOk: false,
                message: 'Bad file format.' + req.body?.data,
            };
        }
